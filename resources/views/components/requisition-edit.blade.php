<span id="{{$id2}}" class="@if($hidden) hidden @endif">
 @if( isset($type) )
  ...
  @endif
 @else
 <input name="{{$name}}" type="text" class="form-control" placeholder="{{$placeholder}}" value="{{$value}}" onchange="assign_new_val(this.value,'{{$id1}}')" onfocusout="toggleShow('{{$id2}}','{{$id1}}')" @if( isset( $keyup ) ) @if( $keyup ) onkeyup="search_product(this.value,'{{$id1}}',1)" @endif @endif>
